context("Test the model bulding interface between R and C")

test_that("C_newSpeciesTable creates a species table in C", {
  test <- .C_newSpeciesTable(10)
  expect_false(is.null(test))
  expect_true(.C_isSpeciesTable(test))
})

test_that("C_numSpecies reports the correct number of species for a species table", {
  test <- .C_newSpeciesTable(10)
  expect_equal(.C_numSpecies(test), 10)
})

test_that("C_getSpeciesValues gets the correct values", {
  test <- .C_newSpeciesTable(10)
  expect_false(.C_setSpeciesValues(test, 50, c(0.1, 0.2, 0.3, 0.4)))
  expect_true(.C_setSpeciesValues(test, 5, c(0.1, 0.2, 0.3, 0.4)))
  expect_true(.C_setSpeciesValues(test, 6, c(0.1, NA_real_, 0.3, 0.4)))
  values <- .C_getSpeciesValues(test, 2)
  expect_equal(length(values), 4)
  expect_equal(values[1], NA_real_)
  expect_equal(values[2], NA_real_)
  expect_equal(values[3], NA_real_)
  expect_equal(values[4], NA_real_)
  values <- .C_getSpeciesValues(test, 5)
  expect_equal(length(values), 4)
  expect_equal(values[1], 0.1)
  expect_equal(values[2], 0.2)
  expect_equal(values[3], 0.3)
  expect_equal(values[4], 0.4)
  values <- .C_getSpeciesValues(test, 6)
  expect_equal(length(values), 4)
  expect_equal(values[1], 0.1)
  expect_equal(values[2], NA_real_)
  expect_equal(values[3], 0.3)
  expect_equal(values[4], 0.4)
})

test_that("we can get/set Predator/Prey Coefficients", {
  test <- .C_newSpeciesTable(10)
  expect_false(.C_setPredatorCoefficients(test, 50, c(1, 2), c(0.1, 0.2)))
  expect_false(.C_setPreyCoefficients(test, 50, c(1, 2), c(0.1, 0.2)))
  expect_false(.C_setPredatorCoefficients(test, 2, c(100, 2), c(0.1, 0.2)))
  expect_false(.C_setPreyCoefficients(test, 2, c(100, 2), c(0.1, 0.2)))
  expect_true(.C_setPredatorCoefficients(test, 5, c(1, 2), c(0.1, 0.2)))
  expect_true(.C_setPreyCoefficients(test, 5, c(1, 2), c(0.1, 0.2)))
  expect_true(.C_setPredatorCoefficients(test, 6, c(3, 4), c(0.3, 0.4)))
  expect_true(.C_setPreyCoefficients(test, 6, c(3, 4), c(0.3, 0.4)))
  values <- .C_getPredatorCoefficients(test, 2)
  expect_true(is.null(values))
  values <- .C_getPredatorCoefficients(test, 5)
  expect_false(is.null(values))
  expect_equal(length(values), 2)
  expect_equal(length(values[[1]]), 2)
  expect_equal(values[[1]][1], 1)
  expect_equal(values[[1]][2], 2)
  expect_equal(length(values[[2]]), 2)
  expect_equal(values[[2]][1], 0.1)
  expect_equal(values[[2]][2], 0.2)
  values <- .C_getPreyCoefficients(test, 6)
  expect_false(is.null(values))
  expect_equal(length(values), 2)
  expect_equal(length(values[[1]]), 2)
  expect_equal(values[[1]][1], 3)
  expect_equal(values[[1]][2], 4)
  expect_equal(length(values[[2]]), 2)
  expect_equal(values[[2]][1], 0.3)
  expect_equal(values[[2]][2], 0.4)
})