...
Let $\overline{S}$ denote the collection of all species. Let $\overline{S}^+_{S_i}$ and $\overline{S}^-_{S_i}$ denote the collection of species which are, respectively, consumed by or consume a given species $S_i$. For any base species, $S_i$, which only consumes some external energy, the set $\overline{S}^+_{S_i}$ will be empty. Conversely for a top predator, $S_j$, in any trophic consumption chain, the set $\overline{S}^-_{S_j}$ will also be empty. For any other species, these two sets will generally contain one or more species. For a given predator-prey or consumer-resource interaction between species, $S_c$ and $S_r$ respectively, we will have $S_c \in \overline{S}^-_{S_r}$ and $S_r \in \overline{S}^+_{S_c}$.

Using these symbols, we can state the rate of change for a given species, $S_i$ as:

$$
\frac{dS_i}{dt} =
  G_e(S_i) + G_c(\overline{S}^+_{S_i})
  - D_p(\overline{S}^-_{S_i}) - D_n(S_i)
$$
...
$$
G_c(S_i) =
  \left( \sum_{j \in \overline{S}^+_{S_i}} e_{i,j} * a_{i,j} * S_j \right)
  * \frac{S_i}{ h_i + \sum_{j \in \overline{S}^+_{S_i}} a_{i,j} * S_j }
$$

Where $h_i$ is the half-saturation parameter for species $S_i$, and $e_{i,j}$ and $a_{i,j}$ are, respectively, the efficiency or conversion and attack rates of the predation of species $S_i$ on species $S_j$. For a given predator, the half-saturation parameter, $h_i$, can be normalized to $1$ by adjusting the efficiency or conversion rates, $e_{i,j}$ for the predator, $S_i$. We will assume that his has been done.
...
$$
D_p(S_i) = \sum_{j \in \overline{S}^-_{S_i}} a_{i,j} * S_i
  * \frac{S_j}{h_j + \sum_{k \in \overline{S}^+_{S_j}} a_{j,k} * S_k}
$$
...
\frac{dS_i}{dt}
  ...
  &   &  + \left( \sum_{j \in \overline{S}^+_{S_i}} e_{i,j} * a_{i,j} * S_j \right)
          * \frac{S_i}{ h_i + \sum_{j \in \overline{S}^+_{S_i}} a_{i,j} * S_j } \\
  &   &  - \sum_{j \in \overline{S}^-_{S_i}} a_{i,j} * S_i
          * \frac{S_j}{h_j + \sum_{k \in \overline{S}^+_{S_j}} a_{j,k} * S_k} \\
  &   &  - m_i * S_i
\end{align*}
$$
